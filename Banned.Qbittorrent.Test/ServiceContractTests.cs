...
public class ServiceContractTests
{
    ...
    public async Task TorrentOperations_UseDocumentedContractsAndPreserveExportBytes()
    {
        ...
        using (httpClient)
        {
            ...
            Multiple(() =>
            {
                ...
                That(handler.Requests.Select(request => request.Uri.AbsolutePath), Is.EqualTo(new[]
                {
                    "/api/v2/torrents/count",
    ...
    [Test]
    public void VersionRange_RejectsEndpointAtRemovedVersionBeforeSendingRequest()
    {
        var (netService, httpClient, handler) =
            CreateNetService(_ => StubHttpMessageHandler.JsonResponse("[]"), new ApiVersion(2, 6));
        using (netService)
        using (httpClient)
        {
            var versionRange = new ApiVersionRange(new ApiVersion(2, 1, 1), new ApiVersion(2, 6));

            ThrowsAsync<Banned.Qbittorrent.Exceptions.QbittorrentEndpointRemovedException>(async () =>
                await netService.Get("/api/v2/search/categories", versionRange));
            That(handler.Requests, Is.Empty);
        }
    }
